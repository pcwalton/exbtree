...
use std::iter::{FromIterator, Peekable, FusedIterator};
use std::marker::PhantomData;
use std::ops::Index;
use std::{fmt, intrinsics, mem, ptr};
...
impl<K, V> BTreeMap<K, V> {
    ...
    /// Returns a cursor allowing you to manually walk the tree.
    ///
    /// This is an `exbtree` addition.
...
/// A cursor allowing you to manually walk the tree.
///
/// This is an `exbtree` addition.
...
impl<'a, K: 'a, V: 'a> Cursor<'a, K, V> {
    ...
    pub fn ascend(&mut self) {
        self.node = match self.node.ascend() {
            Ok(edge) => edge.into_node().forget_type(),
            Err(_) => panic!("Cursor::ascend() called on root node!"),
        }
    }

    #[inline]
    pub fn descend(&mut self, index: usize) {
        assert!(index < self.node.len());
        match self.node.force() {
            Internal(node) => self.node = Handle::new_edge(node, index).descend(),
            Leaf(_) => unreachable!(),
        }
    }
}
